...
public class TestDriveController {
    ...
    // === CHI TIẾT (CHỈ MANAGER) ===
    @GetMapping("/detail/{id}")
    public String detail(@PathVariable Long id, Principal principal, Model model) {
        // Nếu là Staff -> redirect về staff list
        if (userService.hasRole(principal, "DEALER_STAFF")) {
    ...
    public void exportIcs(@PathVariable Long id, HttpServletResponse response) throws IOException {
        ...
        String startUtc = td.getScheduleAt().atZone(ZoneId.systemDefault()).withZoneSameInstant(ZoneOffset.UTC).format(fmt);
        String endUtc = td.getScheduleAt().plusHours(1).atZone(ZoneId.systemDefault()).withZoneSameInstant(ZoneOffset.UTC).format(fmt);
